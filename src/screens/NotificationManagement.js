...
const NotificationManagement = ({route}) => {
      ...
      [
        ...
        },
        {
          ...
          onPress: () => {
            setIsLoading1(true);
            httpClient
              .delete(`notification/delete/${item1.notificationId}`)
              .then(response => {
                if (response.data.status === true) {
                  NOTIFY_MESSAGE(response.data.message);
                  setAllUserData(prevList =>
                    prevList.filter(
                      item => item.notificationId !== item1.notificationId,
                    ),
                  );
                } else {
                  NOTIFY_MESSAGE(response?.data?.message);
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: 'Edit',
          onPress: () => {
            navigation.navigate('AddUpdateNotification', {
              data: item1,
              isEdit: true,
            });
          },
  ...
  return (
    <KeyboardAvoidingView
      ...
      <CustomHeader
        ...
        title={item?.name || 'Notification Management'}
      />
      <View style={styles.headerContainer}>
        ...
        <TouchableOpacity
          onPress={() => {
            ...
            navigation.navigate('AddUpdateNotification', {data, isEdit: false});
          }}
